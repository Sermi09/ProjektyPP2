...
VectoredList::VectoredList(VectoredList &&v) noexcept : head(v.head), tail(v.tail), size(v.size), capacity(v.capacity){
    v.head = nullptr;
    v.tail = nullptr;
    v.size = 0;
    v.capacity = 0;
...
VectoredList& VectoredList::operator=(VectoredList &&v){
    if(this != &v){
        swap(this->head, v.head);
        swap(this->tail, v.tail);
        swap(this->size, v.size);
        swap(this->capacity, v.capacity);
...
VectoredList VectoredList::operator+(const VectoredList &rhs) const {
    ...
    for (size_t index = 0; index < rhs.get_size();++index)
